...
Status — **proof of concept working, captured as regression tests** (`bridge/bridge_test.go`, build tag `cgo`). The suite imports `math`, `builtins`, and `pydantic_core` (a Rust extension): it round-trips `math.sqrt(16)→4.0`, exercises `len([...])`, `int("ff", base=16)→255` (keyword args), nested-dict conversion, `list([1,2])` len/bool/repr, and the headline path — `SchemaValidator({"type": "int"}).validate_python("42")→42` with the failure path surfacing the Python exception as a Go `error`. `TestPydanticCore` skips cleanly when the library isn't installed, so the suite stays green on machines without it.

Bridge API surface:
- `Init` — one-shot `Py_Initialize`, GIL released after so per-call code re-acquires it.
- `Import(name)` — import a module by dotted name.
- `(*Object)`: `.Attr(name)`, `.Call(args...)`, `.CallKw(args, kwargs)`, `.CallMethod(name, args...)`, `.CallMethodKw(name, args, kwargs)`, `.GetItem(key)`, `.Len()`, `.Bool()`, `.Str()`, `.Repr()`, `.Go()` (convert back to a native Go value), `.DecRef()`.
- `toPy` / `fromPy` conversions for `nil`/`bool`/`int`/`int64`/`float64`/`string`/`[]any`/`map[string]any`; unknown Python objects fall back to their `str()`.
- GIL handling is coarse: a single mutex serializes all interpreter access and each call does `PyGILState_Ensure`/`Release`.

**Vendoring infra — done.** `bridge.MainPackageSource()` (in `bridge/embed.go`, no build tag — embeds `bridge.go` via `go:embed`) returns the bridge implementation rewritten as `package main` with the `//go:build cgo` constraint stripped, ready to drop into a generated build directory. The gopy tool itself never links libpython — it embeds the source as text; only the generated binary links it, when `gopy build` writes the vendored file and compiles with CGO enabled. Validated end-to-end: a generated `package main` that vendors this source and calls `Import("math").CallMethod("sqrt", 81.0)` builds with `CGO_ENABLED=1` and prints `9`. Covered by `TestMainPackageSource` (rewrite assertions, runs without CGO).

**Codegen wiring — leaf calls landed (opt-in).** `gopy build -bridge <file.py>` enables `Options.EnableBridge`: a bare `import X` whose module is neither a stdlib shim nor a local sibling (`Options.LocalModules`) is marked bridged in `buildAliases`. A call `mod.fn(args, kw=v)` on a bridged name lowers to a `MethodCall` whose receiver is the module name; `methodCall` detects the bridged receiver and emits `__bridgeGo(__bridgeModule("mod").CallMethod("fn", args...))` (or `CallMethodKw` with a `map[string]any` of keywords). `__bridgeModule` lazily imports + caches the module; `__bridgeGo` unwraps the result, panics on a Python-side error, and converts to a native Go value. Args are boxed to their canonical Go widths so they cross the `...any` API. When any bridged call is emitted the transpiler sets `ModuleMeta.UsedBridge`, and `gopy build` then vendors `bridge.MainPackageSource()` as `gopy_bridge.go` and builds with `CGO_ENABLED=1`; without bridged calls the binary stays pure Go.
...
**Recursive transpilation with per-module bridge fallback — landed.** `gopy build <entry.py>` discovers the local import graph (`discoverLocalModules`): each `import X` / `from X import …` is resolved to a local `.py` — a sibling (`utils.py`), a subpackage module (`pkg/sub.py`), or a package init (`pkg/__init__.py`, via `resolveModuleFile`) — parsed for *its* imports, and so on. Import forms handled: `from pkg import mod; mod.fn()` (resolves `pkg/mod.py`, strips `mod.`), `from pkg.sub import name` (bare `name`), `import pkg.mod; pkg.mod.fn()` (deep-dotted qualifier strip, via `dottedName`), and **relative imports** (`from .util import f`, `from . import util`, `from ..pkg import x`) resolved against the importing module's own package dir (ascending one level per leading dot). Two-phase build:

1. **Pure recursion** — the entry and every discovered module are transpiled into one shared `package main` (helpers consolidated in `gopy_runtime.go`; each module → a uniquely-named `.go`), with `LocalModules` set so qualified `mod.fn(...)` / `pkg.sub.fn(...)` / `mod.NAME` accesses drop the qualifier (the whole project is one Go package). If this builds, the binary is pure Go — no CPython, no CGO, statically linked.

**Per-module symbol mangling — landed (the flat-package collision fix).** Each module's top-level free functions and module vars emit as `<modpath dots→_>_<name>` (e.g. `mypkg/calc.py`'s `add` → `mypkg_calc_add`), so two modules defining the same symbol no longer collide in the shared package. Chosen over one-Go-package-per-module because Go forbids circular package imports — which real Python libraries routinely have — whereas one flat package tolerates them. How the three hard parts were handled: (a) **scope-aware references** — mangling lives in codegen, which already owns the global-vs-local decision (`g.globals` + the `:=`/`=` choice + `localVarTypes`), so only module-level refs mangle and a local/param that shadows a global stays bare; (b) **type-system ripple sidestepped** — classes stay bare (mangling a type name would cascade through `goType`/constructors/receivers/enum members/ClassVars/isinstance), so cross-module class-name collisions remain the one documented limitation; (c) **driver-resolved bindings** — the build driver resolves each import (absolute, relative, aliased, submodule) to the defining module's path and passes pre-mangled symbols into codegen (`Options.ImportedSymbols`/`ModuleBindings`/`ModulePath`/`LocalModulePaths`), so `from a import f as g` resolves `g` → `a_f` and relative-import targets keep their full path. Gated on a multi-module build — single-module builds set no `ModulePath`/`LocalModulePaths`, mangle nothing, and are byte-for-byte unchanged (the 653-fixture golden suite calls `transpile.Module` directly and never mangles). Verified: two modules each defining `helper()`+`VAL` build into one pure-Go binary and print `A B 1 2` (matching CPython); covered by `TestCLIBuildSymbolCollision`. **Residual limitation:** two modules defining a class of the same name still collide (classes are bare) — rare; bridge fallback or a rename works around it.
...
Verified: a typed sibling (`import helper; helper.shout(...)` + `from helper import GREETING`) transpiles to pure Go and matches CPython; a **subpackage** (`from mypkg.calc import add` + `from mypkg import calc; calc.mul(...)`) transpiles to a statically-linked pure-Go binary with **no libpython** and matches CPython; an untyped sibling used as a bridged base (`class Child(baselib.Base)`, `baselib` untyped) trips the pure build, falls back, and runs `Child(7).shout()` → `HI 7` via embedded CPython. Covered by `TestCLIBuildRecursive`, `TestCLIBuildSubpackage`, `TestCLIBuildRelativeImports`, `TestCLIBuildDottedImport`, and the `geometry` multi-file fixture.

**The goal — "all Go + Python libraries, no interpreter" — and its honest ceiling.** A pure-Python library written in the supported typed subset *can* now be transpiled recursively to Go and linked into a static binary with zero CPython (the subpackage demo above proves the shape). What *cannot* be eliminated: a compiled C-extension `.so` (pydantic_core, numpy, psycopg2's C core) is bound to the CPython ABI — it calls back into libpython for refcounting / type objects / the GIL on nearly every operation, so it can't run "through a Go interface" without that interface *being* an embedded CPython. The bridge already is that interface. So the realistic decomposition of any real app: pure-Python (app + subset-conforming libs) → transpiled Go, no interpreter; C-extension leaves → either bridged (interpreter present) or reimplemented in Go per-library (as `-goweb` did for FastAPI/Flask routing+validation). Recursive transpilation shrinks the interpreter's footprint to exactly those C-extension leaves; reaching *zero* interpreter requires every dependency to be either subset-Python or Go-reimplemented.
...
**Iteration — landed.** `for x in obj` over a bridged value materializes the Python iterable into a `[]any` via `*Object.Iter()` (the iterator protocol — `iter()` / `next()`) and ranges over it. Verified `for x in mymod.make_list(): print(x)` → `10 / 20 / 30`. Loop elements are `any`, so they print and pass to other bridged calls fine.

**Working bridge surface (all `-bridge`, all verified e2e against CPython):** module calls `mod.fn(a, kw=v)`; method chains `mod.Cls({...}).validate_python("42")` (incl. the real pydantic-core Rust extension); attribute reads `mod.CONST`; `from X import Y` of bridged modules; subscript `obj[key]`; iteration `for x in obj`; and Python exceptions catchable by type (`except ValueError`). Seven end-to-end demos cover these.
...
The forward bridge lets Go call Python; the **reverse** bridge (`bridge/reverse.go`, build tag `cgo`) lets Python call Go — the direction frameworks need (a web framework / callback registry / plugin host invoking user code). `bridge.RegisterFunc(fn func([]any) any) (*Object, error)` exposes a Go callback as a callable Python object: each callback gets an integer id, and a shared C trampoline (a `PyCFunction` whose bound `self` carries the id) hands the Python arg tuple to an `//export`-ed Go dispatcher (`gopyRevDispatch`), which converts args via `fromPy`, runs the callback, and converts the result via `toPy`. A panic in the callback is recovered and surfaced as a Python `RuntimeError` instead of crashing the interpreter.

Captured as `bridge/reverse_test.go`: Python's `map(go_doubler, [1,2,3])` → `[2,4,6]` (Python driving the Go callback per element); a Go→callable→Go round trip; keyword arguments (`fn(10, step=5)` → `15`); and the panic→Python-error path. The trampoline uses `METH_VARARGS | METH_KEYWORDS`, so a registered callback receives both positional `args []any` and `kwargs map[string]any`.
...
**Rich object handles.** `fromPy` now returns an unrecognized Python object (a class instance, module, function — anything not None/bool/int/float/str/list/tuple/dict) as a live `*Object` instead of its `str()`. Go code receives arbitrary Python values as usable handles it can read attributes / call methods on / pass back. This is what lets a reverse callback receive `self` (or any rich arg) and operate on it. Verified (`bridge/reentrant_test.go`): a `Counter` instance handed to Go, `.inc()` called on it through the handle.

**Reentrancy + concurrency — `gilMu` removed.** The old coarse `sync.Mutex` was redundant (the GIL itself serializes — `PyGILState_Ensure` blocks until the thread owns the interpreter) and *harmful* (non-reentrant, so a reverse callback re-entering the forward bridge deadlocked). `withGIL` now relies on the GIL alone (still `runtime.LockOSThread`-pinned for thread-state correctness); `PyGILState_Ensure`/`Release` is recursive, so Python → Go → Python re-entry works. Verified: `TestReentrantBridge` (a Go callback invoked from Python calls `.inc()` back through the bridge → no deadlock) and `TestConcurrentGIL` (50 goroutines hammering the bridge concurrently stay correct). Bridge package: 20/20 runs green.
...
**Framework integration proven end-to-end.** `TestFrameworkDispatch` builds the exact pattern a web framework uses per request: a Python dispatcher calls `inspect.signature(handler)`, reads each parameter's annotation, coerces the raw (string) request params to those types, and invokes the handler. A Go-backed `add(a: int, b: int)` dispatched with `{"a": "3", "b": "4"}` returns `7` — the framework introspected the transpiled Go handler, coerced strings to ints from its annotations, and called it. This is the core of FastAPI's request binding, working against Go code.

**Go-backed classes — Pydantic models from Go work.** `bridge.MakeClass(name string, bases []*Object, fields []Field) (*Object, error)` creates a Python class with annotated fields, built through the most-derived base **metaclass** — so inheriting `pydantic.BaseModel` runs Pydantic's `ModelMetaclass` and produces a real validating model, declared entirely from Go. Verified (`bridge/class_test.go`): `MakeClass("User", [BaseModel], [name:str, age:int])` then from Python `User(name="ana", age="3")` validates and coerces (`age` `"3"`→`3`), `u.name`/`u.age` read back correctly, and a bad `age` raises Pydantic's `ValidationError`. This clears the metaclass blocker that made `class User(BaseModel)` impossible before.

The framework-integration primitives are now all proven against real libraries: call any Python library (forward), expose Go functions to Python with a real signature (`RegisterTypedFunc` + `inspect.signature`), the per-request introspect→coerce→call pattern (`TestFrameworkDispatch`), and Go-declared Pydantic models (`MakeClass`).

**Richer annotations — landed.** The annotation resolver is now `eval`-based over a namespace with the scalars, the `typing` generics (`Optional`, `List`, `Dict`, `Union`, `Any`), and the lowercase builtin generics — so `list[int]`, `Optional[str]`, `dict[str, int]` resolve in a signature. `bridge.RegisterType(name, obj)` registers a Go-declared type (e.g. a `MakeClass` model) under a name so handler annotations can reference it. Verified (`bridge/annotations_test.go`): `f(xs: list[int], name: Optional[str]) -> dict[str, int]` reports the right signature, and a handler annotated with a Go-registered `Widget` resolves that class via `get_type_hints`.

### Transpiler wiring (auto-emit from Python source)
...
- `str.format("{} ...")` with positional `{}` placeholders. **`str.format_map(d)`** also wired — `d` is any `dict[str, *]` shape (auto-coerced through reflection) and routes through the same `__gopy_str_format` helper
- Nested function bodies that end with a `try`-with-return now get the same fall-through guard top-level funcs already had (synthetic `panic("gopy: try fell through without returning")`). Builtin coerce helpers `int(s)` / `float(s)` now raise `ValueError` via the `NewException` channel on bad input, so `try: int(s) except ValueError:` patterns work end-to-end
- Enum variable typing inferred from `EnumClass.MEMBER` initializers: `c = Color.RED` now propagates `Color`'s type into the local-var registry so `c.name` / `c.value` routes through the enum-aware attribute codegen without an explicit annotation
- Nested function declarations in the same block can now reference each other in any textual order: an enclosing-block pre-pass forward-declares every `var name func(...) ret` and registers the func in the symbol table before bodies emit. Mutual recursion + forward calls between siblings (`def a(): return b(); def b(): return 1`) compile cleanly
...
- Strided slice assignment `xs[start:stop:step] = rhs` (was rejected): emits a loop that walks rhs in parallel with the indices `start, start+step, ...`. Negative `step` defaults missing `start`/`stop` to `len-1` / `-1` (matching CPython), and negative literals on supplied bounds get normalized via `+= len`
- `dict.get(k, {})` / `dict.get(k, [])` with an empty list/dict default now reshapes the literal to the dict's value type so the inferred IIFE return matches `dict[K, V]` rather than `map[any]any` / `[]any`
- Nested class definitions inside a function body: a pre-pass hoists `class Foo: ...` declarations from function bodies to module scope before lowering, so the inner class becomes a top-level Go struct + methods. Closure capture over enclosing locals isn't reproduced — methods only see the class instance and module-level state
- Positional `*`-spread of a list literal in a call: `add(*[1, 2, 3])` expands to `add(1, 2, 3)` at IR-lower time. Name-based spreads (`f(*args)` where `args` is a variable) still require explicit positional forms
- Nested-subscript `del`: `del d[k][i]` / `del grid[r][c]` work for any list-typed receiver (the rewrite re-emits the inner subscript on both sides of the `append(..., ...+1:)` rewrite)
...
- Default parameter values: `def f(a: int, b: int = 5)` — defaults are evaluated at every call site (so mutable defaults can't leak between calls). User-class `__init__` defaults are now extracted the same way: `Box()` with `__init__(self, value=0, label="")` now fills missing trailing args (previously only @dataclass synthesis honored defaults)
...
- List comprehensions `[expr for var in iter [if cond]]`, dict comprehensions `{k: v for var in iter [if cond]}`, and generator expressions `(expr for var in iter [if cond])` (multiple `for`+`if` clauses supported; the gen-expr form materializes eagerly). Multiple `if` filters per generator (`[x for x in xs if a if b]`) now AND together — previously only one filter was accepted
...
- Dict / list comprehensions accept tuple-unpack targets over `dict.items()`: `{v: k for k, v in d.items()}` and `[f"{k}={v}" for k, v in d.items()]` lower to a `for k, v := range d` loop inside the IIFE, with the inner expression seeing both bindings typed from the dict's key/value annotations. **R62**: the tuple-target path also accepts any typed list of 2-tuples (`list[tuple[K, V]]` / `list[list[T]]`) — see the dedicated entry near the end of this section
...
- **`del name`** — accepted as no-op (Go scope handles binding removal); `del d[k]` and `del xs[i]` work. Nested forms `del d[k][i]` / `del grid[r][c]` now also compile (the LHS re-emits the inner subscript on both sides of the `append` rewrite)
...
- **`urllib.robotparser.RobotFileParser`** real-ish: `__RobotFileParser` with `.set_url(url)`, `.read()` (fetches the robots.txt via `net/http` + parses User-agent / Allow / Disallow blocks), `.can_fetch(agent, url)` (walks rules per-agent then falls back to `*`). `.crawl_delay`, `.request_rate`, `.site_maps`, `.mtime`, `.modified` are call-shape stubs. Before `.read()` runs, `.can_fetch` returns `False` to match CPython's safe-default; on fetch failure it switches to `True` (CPython treats network errors as unrestricted)
- **`string.Formatter()`** real-ish class: `__StringFormatter` with `.format(tmpl, *args, **kwargs)` walking `{0}` / `{name}` / `{}` placeholders (auto-numbered when empty), `{{` / `}}` literal escapes, and `:spec` suffix stripping (full spec rendering still collapses to `fmt.Sprintf("%v", ...)`). Also exposes `.vformat`, `.get_field`, `.get_value`, `.parse`, `.check_unused_args`, `.format_field`, `.convert_field` as call-shape stubs so subclass-friendly call chains compile
