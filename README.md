...
_Generated by `scripts/update_bench.sh` on 2026-05-20T11:44:34Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 49.01 | 1.73 | 28.3x | 12.83 | 4.46 | 2.88x |
| bench_fib | 139.14 | 5.69 | 24.5x | 12.78 | 4.54 | 2.81x |
| bench_loop | 108.92 | 1.87 | 58.2x | 12.80 | 4.77 | 2.68x |
...
- [x] `csv.writer(fh)` stateful writer with `.writerow(row)` / `.writerows(rows)` bound to a `with open(...)` handle; `csv.DictReader(lines)` returns `[]map[string]string`
