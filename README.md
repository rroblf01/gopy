...
_Generated by `scripts/update_bench.sh` on 2026-05-21T05:59:16Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 46.79 | 1.47 | 31.8x | 12.90 | 4.80 | 2.69x |
| bench_fib | 134.05 | 5.31 | 25.2x | 12.87 | 4.66 | 2.76x |
| bench_loop | 105.93 | 1.79 | 59.2x | 12.90 | 4.63 | 2.79x |
...
- [x] Operator overloading via dunder methods: `__add__` / `__sub__` / `__mul__` / `__truediv__` / `__floordiv__` / `__mod__` / `__pow__` and `__lt__` / `__le__` / `__gt__` / `__ge__` / `__eq__` / `__ne__` route `a + b` / `a < b` etc. through the method when LHS is a registered user class
- [x] Container dunder methods: `__getitem__` (drives `obj[k]`), `__setitem__` (drives `obj[k] = v`), `__contains__` (drives `x in obj` / `x not in obj`)
- [x] Builtin dispatch to dunders: `bool(obj)` → `__bool__`, `len(obj)` → `__len__`, `abs(obj)` → `__abs__`, `str(obj)` → `__str__`, `int(obj)` → `__int__`, `float(obj)` → `__float__`, `reversed(obj)` → `__reversed__`, `hash(obj)` → `__hash__`, `obj(args)` → `__call__` (also recognized: `__neg__` / `__pos__` / `__iter__` / `__next__`)
