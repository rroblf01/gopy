...
_Generated by `scripts/update_bench.sh` on 2026-05-20T05:48:48Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 52.89 | 1.87 | 28.3x | 12.85 | 4.51 | 2.85x |
| bench_fib | 147.96 | 5.99 | 24.7x | 12.85 | 4.47 | 2.87x |
| bench_loop | 114.45 | 2.12 | 54.0x | 12.75 | 4.52 | 2.82x |
...
- [x] `math` (trig, log, sqrt, floor/ceil-as-int, trunc, fmod, gcd, isnan/isinf/isfinite, copysign, hypot, degrees/radians, constants `pi`/`e`/`tau`/`inf`/`nan`)
...
- [x] `list.count(x)`, `list.index(x)` (raises ValueError when missing), `list.extend(ys)`, `list.insert(i, v)`, `list.remove(v)`, `list.clear()`, `list.sort([reverse=True])`, `list.reverse()`
- [x] `str.isdigit()`, `.isalpha()`, `.isalnum()`, `.isspace()`, `.isupper()`, `.islower()`
- [x] `str.split(sep, maxsplit)`, `str.splitlines()`, `str.partition(sep)`, `str.rpartition(sep)`
...
- [x] `dict.update(other)`, `dict.pop(key[, default])` (raises KeyError when missing and no default), `dict.setdefault(key, default)`, `dict.clear()`
