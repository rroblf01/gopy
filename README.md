...
- Builtins: `print`, `len`, `str`, `int`, `float`, `range`, `sorted`, `sum`, `min`, `max`, `any`, `all`, `reversed`, `abs`, `round`, `isinstance` (single class or tuple-of-classes), `issubclass`, `pow`, `chr`, `ord`, `repr`, `divmod`, `getattr`, `setattr`, `hasattr`, `list` (iterator materialization — pass-through in the gopy shim), `iter`, `next(it[, default])`, `id`, `hash`
...
- Union annotations (`int | str`, etc.) lower to `any`; combine with `isinstance` to narrow at the call site
...
_Generated by `scripts/update_bench.sh` on 2026-05-18T07:45:00Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 46.74 | 1.61 | 29.0x | 12.94 | 4.42 | 2.93x |
| bench_fib | 133.60 | 5.34 | 25.0x | 12.95 | 4.46 | 2.90x |
| bench_loop | 108.89 | 1.92 | 56.7x | 12.84 | 4.48 | 2.87x |
...
- [ ] `Optional[int]` / `Union[...]` (typing module forms)
...
- [ ] `itertools.groupby`, full-arity `combinations` / `product`
