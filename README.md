...
_Generated by `scripts/update_bench.sh` on 2026-05-20T06:44:09Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 45.80 | 1.47 | 31.2x | 12.88 | 4.38 | 2.94x |
| bench_fib | 133.90 | 5.31 | 25.2x | 12.90 | 4.51 | 2.86x |
| bench_loop | 105.00 | 1.74 | 60.3x | 12.82 | 4.51 | 2.84x |
...
- [x] `pathlib.Path.iterdir()` (returns `[]*Path`; loop var inherits the Path tag so `for child in p.iterdir(): print(child.name)` works), `.mkdir(parents, exist_ok)`, `.unlink()`
- [x] `json.dumps(v, indent=N)` pretty-prints with N-space indentation; default form keeps the existing Python-style separators
- [ ] `pathlib.Path` globbing (`p.glob("*.txt")`)
