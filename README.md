...
- [x] `subprocess.run` returning a typed `CompletedProcess`. Honored kwargs: `input=` (pipes the string into the child's stdin via `strings.NewReader`), `cwd=` (sets `cmd.Dir`). Other kwargs (`capture_output`, `text`, `check`, ...) are accepted at the call site and silently dropped since Go's exec captures stdout / stderr unconditionally
...
- [x] `random` (`random()`, `randint(a, b)`, `uniform(a, b)`, `seed(n)`, `choice(xs)`, `shuffle(xs)` in-place, `sample(xs, k)`, `choices(xs[, weights=ws, k=N])` — k=1 default; with weights, biases the draw via a cumulative-distribution lookup over `math/rand.Float64`)
...
- [x] `secrets.token_hex([n])`, `secrets.token_urlsafe([n])`, `secrets.token_bytes([n])` (CSPRNG via `crypto/rand`), `secrets.choice(seq)` (uniform pick from a typed slice via `crypto/rand.Int`; panics `IndexError` on empty)
...
- [x] `tempfile.mkdtemp([prefix])`, `tempfile.mkstemp([prefix])` (returns `[fd, name]`), `tempfile.gettempdir()`, `with tempfile.TemporaryDirectory([prefix=...]) as d:` lowers to an IIFE that creates a fresh `os.MkdirTemp` directory, binds its path to `d` (string), and defers `os.RemoveAll` so cleanup fires even on panic. `with tempfile.NamedTemporaryFile([mode=, prefix=, suffix=, delete=]) as f:` creates an `os.CreateTemp` file, tags the binding `__NamedTempFile` so `f.name` / `f.write(s)` / `f.read()` / `f.seek(off)` / `f.flush()` / `f.close()` dispatch through the wrapper, and defers Close + Remove (unless `delete=False`)
...
- [x] `urllib.request.urlopen(url[, data])` HTTP GET / POST returning an `HTTPResponse` analog (`.read()` / `.status` / `.headers` / `.getcode()` / `.close()`). A positional or `data=` string body switches to POST with `application/x-www-form-urlencoded`; passing the body as `bytes` also works since gopy maps `bytes` to `string`. `http.client.HTTPConnection` / `HTTPSConnection` with `.request(...)` / `.getresponse()`. `http.server`'s `BaseHTTPRequestHandler` is registered as a stub — real request handling lives outside the gopy shim layer (write a Go-side `net/http` handler if needed)
...
- [x] `os.getcwd()`, `os.listdir(p)`, `os.makedirs(p[, exist_ok])`, `os.mkdir(p)`, `os.rmdir(p)`, `os.remove(p)`, `os.rename(src, dst)`, `os.scandir(p)` (eager-materialized `[]*__DirEntry`; entries expose `.name` / `.path` and `.is_file()` / `.is_dir()` / `.is_symlink()`), `os.sep`, `os.linesep`
- [x] `os.path.join`, `os.path.exists`, `os.path.isfile`, `os.path.isdir`, `os.path.basename`, `os.path.dirname`, `os.path.splitext`, `os.path.abspath`, `os.path.split`, `os.path.relpath`, `os.path.getsize`, `os.path.getmtime`, `os.path.getatime`, `os.path.getctime` (Go has no portable atime/ctime — both collapse to mtime), `os.path.normpath`, `os.path.expanduser`, `os.path.expandvars`, `os.path.commonprefix`, `os.path.samefile`
