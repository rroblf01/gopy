...
_Generated by `scripts/update_bench.sh` on 2026-05-21T05:49:41Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 46.60 | 1.55 | 30.1x | 12.90 | 4.95 | 2.61x |
| bench_fib | 134.42 | 5.42 | 24.8x | 12.86 | 4.88 | 2.64x |
| bench_loop | 106.83 | 1.80 | 59.3x | 12.89 | 4.88 | 2.64x |
...
- [x] Builtin dispatch to dunders: `bool(obj)` → `__bool__`, `len(obj)` → `__len__`, `abs(obj)` → `__abs__` (also recognized: `__neg__` / `__pos__` / `__int__` / `__float__` / `__iter__` / `__next__`)
