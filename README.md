...
- `os.environ` — exposed as a `dict[str, str]` snapshot of the process environment (annotate the binding `env: dict[str, str] = os.environ` to keep `in` / `.get()` working). Read-only — mutations don't propagate back to the OS. Also: `os.cpu_count()`, `os.urandom(n)`, `os.path.isabs(p)`
...
_Generated by `scripts/update_bench.sh` on 2026-05-21T06:56:57Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 47.11 | 1.49 | 31.6x | 12.86 | 4.85 | 2.65x |
| bench_fib | 133.42 | 5.36 | 24.9x | 12.86 | 4.73 | 2.72x |
| bench_loop | 107.58 | 1.76 | 61.1x | 12.93 | 4.86 | 2.66x |
...
- [x] `math` (trig, log, sqrt, floor/ceil-as-int, trunc, fmod, gcd, lcm, isnan/isinf/isfinite, copysign, hypot, degrees/radians, factorial, comb, perm, dist, prod (int64), remainder, constants `pi`/`e`/`tau`/`inf`/`nan`)
...
- [x] `abc.ABC` / `abc.ABCMeta` base classes and `@abstractmethod` / `@abc.abstractmethod` / `@abstractclassmethod` / `@abstractstaticmethod` / `@abstractproperty` — accepted; the ABC base drops from the embed list and `...` / docstring-only stub bodies compile to a `NotImplementedError` panic so subclasses must override
