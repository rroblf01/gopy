...
_Generated by `scripts/update_bench.sh` on 2026-05-20T14:23:47Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 46.21 | 1.44 | 32.1x | 12.92 | 4.68 | 2.76x |
| bench_fib | 133.88 | 5.46 | 24.5x | 12.87 | 4.56 | 2.82x |
| bench_loop | 105.58 | 1.79 | 59.0x | 12.78 | 4.86 | 2.63x |
...
- [x] Dunder methods on user classes (`__str__` → Go `String()`, `__repr__` → `Repr()`, `__len__` → `Len()`, `__eq__` → `Eq()`, `__hash__` → `Hash()`); explicit `obj.__str__()` calls rewrite to the Go names, so `print(obj)` triggers the Stringer dispatch automatically
