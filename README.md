...
- Lambda expressions, with specialization at well-known call sites: `map(lambda x: ..., xs)`, `filter(lambda x: ..., xs)`, and `sorted(xs, key=lambda x: ..., reverse=...)` re-lower the body with the iterable's element type so arithmetic / attribute access typechecks. Standalone lambdas fall back to `func(p any) any` and only work when the body operates on `any`-friendly values.
...
_Generated by `scripts/update_bench.sh` on 2026-05-18T06:57:35Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 47.02 | 1.54 | 30.5x | 12.81 | 4.55 | 2.82x |
| bench_fib | 136.57 | 5.42 | 25.2x | 12.83 | 4.51 | 2.84x |
| bench_loop | 106.62 | 1.74 | 61.3x | 12.85 | 4.34 | 2.96x |
...
- [x] `itertools.chain`, `itertools.accumulate`
...
- [x] `urllib.parse.quote` / `unquote` / `urlencode`
