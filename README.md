...
_Generated by `scripts/update_bench.sh` on 2026-05-21T06:05:14Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 47.89 | 1.46 | 32.8x | 12.85 | 4.80 | 2.68x |
| bench_fib | 144.36 | 6.44 | 22.4x | 12.93 | 4.63 | 2.79x |
| bench_loop | 104.43 | 1.98 | 52.7x | 12.78 | 4.76 | 2.68x |
...
- [x] Operator overloading via dunder methods: `__add__` / `__sub__` / `__mul__` / `__truediv__` / `__floordiv__` / `__mod__` / `__pow__` / `__or__` / `__and__` / `__xor__` / `__lshift__` / `__rshift__` / `__matmul__` and `__lt__` / `__le__` / `__gt__` / `__ge__` / `__eq__` / `__ne__` route `a + b` / `a < b` / `a | b` etc. through the method when LHS is a registered user class; unary `-obj` / `+obj` / `~obj` route through `__neg__` / `__pos__` / `__invert__`
- [x] Container dunder methods: `__getitem__` (drives `obj[k]`), `__setitem__` (drives `obj[k] = v`), `__contains__` (drives `x in obj` / `x not in obj`)
- [x] Builtin dispatch to dunders: `bool(obj)` → `__bool__`, `len(obj)` → `__len__`, `abs(obj)` → `__abs__`, `str(obj)` → `__str__`, `int(obj)` → `__int__`, `float(obj)` → `__float__`, `reversed(obj)` → `__reversed__`, `hash(obj)` → `__hash__`, `obj(args)` → `__call__`, and `for v in obj:` → `__iter__` when the method returns a typed list (also recognized: `__next__`)
