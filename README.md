...
- Single **and multiple** inheritance, `super().__init__(...)`, `super().method(...)`; mixin bases are zero-initialized automatically
...
- Stdlib shims: `sys.argv`, `sys.exit(n)`, `os.getenv(k)`, `time.time()`, `time.sleep(s)`, `json.dumps(x)`, `json.loads(s)`, `datetime.datetime.now()`, `re.findall(p, s)`, `re.search(p, s)`, `re.match(p, s)`, `re.sub(p, repl, s)`
...
- Richer stdlib (`pathlib`, `datetime.timedelta`, datetime arithmetic, file iteration, csv)
- `@classmethod` decorators, custom decorators with arguments
- Metaclasses, `__getattr__` / descriptors
- Diamond-pattern multiple inheritance with conflicting methods (relies on Go embedding rules)
- Dynamic features: `eval`, `exec`, monkey-patching, runtime `setattr`
- Generator `send()` / `throw()`, async / `await`
- Match objects from `re.search`/`re.match` returning string instead of Match (no `.group()` / `.groups()`)
- C extensions
...
_Generated by `scripts/update_bench.sh` on 2026-05-17T10:08:37Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 48.76 | 1.59 | 30.7x | 12.88 | 4.21 | 3.06x |
| bench_fib | 137.45 | 5.61 | 24.5x | 12.84 | 4.09 | 3.14x |
| bench_loop | 108.01 | 1.94 | 55.7x | 12.79 | 4.21 | 3.04x |
