...
- Builtins: `print`, `len`, `str`, `int`, `float`, `range`, `sorted`, `sum`, `min`, `max`, `any`, `all`
...
_Generated by `scripts/update_bench.sh` on 2026-05-17T11:22:44Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 46.38 | 1.45 | 32.0x | 12.85 | 4.25 | 3.02x |
| bench_fib | 134.27 | 5.37 | 25.0x | 12.81 | 4.02 | 3.19x |
| bench_loop | 106.07 | 1.77 | 59.9x | 12.86 | 4.33 | 2.97x |
...
- [ ] Tuple literals + tuple unpacking (`a, b = pair`, `for k, v in pairs:`)
- [ ] Multiple-target assignment (`a = b = 0`, `a, b = 1, 2`)
...
- [ ] `map(fn, xs)` and `filter(fn, xs)` (probably as eager list materializers)
- [ ] `zip(a, b)` and `enumerate(xs)` — gated on tuple unpacking landing first
- [ ] `reversed(xs)`
