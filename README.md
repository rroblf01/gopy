...
- `socket` TCP client minimal: constants `AF_INET` / `AF_INET6` / `AF_UNIX` / `SOCK_STREAM` / `SOCK_DGRAM`; `socket.socket(family, type)` returns a `__Socket` wrapping `net.Conn`; methods `.connect((host, port))`, `.send(s)`, `.sendall(s)`, `.recv(n)`, `.close()`. Also usable as a context manager. UDP / Unix streams not actually wired — `Connect` always dials TCP. `socket.create_connection((host, port))` works as a shortcut
...
_Generated by `scripts/update_bench.sh` on 2026-05-21T14:03:38Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 55.03 | 2.54 | 21.7x | 12.78 | 4.85 | 2.64x |
| bench_fib | 152.86 | 5.52 | 27.7x | 12.83 | 4.86 | 2.64x |
| bench_loop | 129.50 | 1.87 | 69.3x | 12.91 | 4.98 | 2.59x |
