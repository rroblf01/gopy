...
- Django-flavored ORM (F9): subclass `Model` from `gopy_django.models` and declare `CharField`, `IntegerField`, `BooleanField` at class scope. The transpiler emits a per-class struct, kwargs constructor, `save()` method, and a `<Class>Objects` Manager singleton with `.all()` / `.filter(**kw)` / `.get(**kw)` / `.create(**kw)`. The same Python shim runs unchanged under CPython, so fixtures round-trip across both runtimes.
...
- C extensions
- Django itself (the long-term target, gated on URL routing, request/response, templating, migrations, and middleware shims — the ORM piece landed in F9)
...
_Generated by `scripts/update_bench.sh` on 2026-05-17T10:48:07Z._

| Benchmark | CPython (ms) | gopy Go (ms) | Speedup | Python RSS (MB) | gopy RSS (MB) | RSS save |
|-----------|-------------:|-------------:|--------:|----------------:|--------------:|---------:|
| bench_class | 49.38 | 1.75 | 28.2x | 12.85 | 4.03 | 3.19x |
| bench_fib | 136.62 | 6.05 | 22.6x | 12.88 | 4.32 | 2.98x |
| bench_loop | 106.43 | 1.91 | 55.7x | 12.86 | 4.27 | 3.01x |
